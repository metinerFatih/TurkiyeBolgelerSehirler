    ...
    public class UygulamaDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            foreach (var item in bolgeler)
                item.Slug = UrlUtilities.URLFriendly(item.BolgeAd);
            ...
            foreach (var item in sehirler)
                item.Slug = UrlUtilities.URLFriendly(item.SehirAd);

            modelBuilder.Entity<Bolge>().HasData(bolgeler);
            modelBuilder.Entity<Sehir>().HasData(sehirler);
        }
    }
}
